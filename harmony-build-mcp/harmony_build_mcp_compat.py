#!/usr/bin/env python3
"""
HarmonyOS Build MCP Server (Python 3.8+ Compatible).
...
Compatible with Python 3.8+. For Python 3.10+, use harmony_build_mcp.py instead.
"""
...
class BuildTarget:
    ...
    @classmethod
    def values(cls):
        return [cls.HAP, cls.HAR, cls.HSP]
...
def get_module_info(project_path: Path) -> Dict[str, Any]:
    """
...
class BuildInput(BaseModel):
    ...
    def validate_target(cls, v: str) -> str:
        if v not in BuildTarget.values():
            raise ValueError(f"Invalid target: {v}. Must be one of {BuildTarget.values()}")
        return v

    @validator('mode')
    @classmethod
    def validate_mode(cls, v: str) -> str:
        if v not in BuildMode.values():
            raise ValueError(f"Invalid mode: {v}. Must be one of {BuildMode.values()}")
        return v

    @validator('output_format')
    @classmethod
    def validate_output_format(cls, v: str) -> str:
        if v not in ResponseFormat.values():
            raise ValueError(f"Invalid output_format: {v}. Must be one of {ResponseFormat.values()}")
        return v
...
def _format_build_result(result: Dict[str, Any], format: str) -> str:
...
async def _execute_build(
    ...
    try:
        ...
        if module:
            cmd.append(f":{module}:assemble{mode}")
        else:
            # Build all modules
            cmd.append(f"assemble{mode}")
...
async def harmony_build(params: BuildInput) -> str:
    ...
    Args:
        params (BuildInput): Validated input parameters containing:
            ...
            - target (str): Build target type (hap/har/hsp)
            - mode (str): Build mode (debug/release)
            - module (Optional[str]): Specific module to build
            - clean (bool): Whether to clean before building
            - output_format (str): Response format (markdown/json)
...
async def harmony_clean(project_path: str, output_format: str = ResponseFormat.MARKDOWN) -> str:
...
async def harmony_get_project_info(project_path: str, output_format: str = ResponseFormat.MARKDOWN) -> str:
...
async def harmony_build_get_status() -> str:
    ...
    status = {
        ...
        "supported_targets": BuildTarget.values(),
        "supported_modes": BuildMode.values()
    }
